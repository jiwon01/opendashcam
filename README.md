...
`Open BlackBox`는 Android에서 동작하는 블랙박스형 영상 녹화 앱입니다.  
CameraX 기반으로 연속 분할 녹화, 워터마크 오버레이, 저장소 정책 관리, 녹화 파일 관리(재생/공유/삭제)를 제공합니다.

## 개발 배경

기존 블랙박스 앱들은 레거시 구조이거나, 속도/시간 등의 메타데이터가 영상에 충분히 삽입되지 않는 문제가 있었습니다.  
특히 시간 메타데이터가 영상에 명확히 포함되어야 공익신고나 사고 발생 시 증빙 자료로 더 유용하게 활용할 수 있다고 판단했고, 이 문제를 해결하고자 `Open BlackBox`를 구현했습니다.
또한 실제 사용 환경을 고려해 디테일한 기능을 사용자가 직접 ON/OFF 할 수 있도록 설정 경험을 설계했습니다.
